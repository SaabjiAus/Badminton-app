# ==============================================================================
# 🏸 BADMINTON CLUBHOUSE LOCAL ENGINE - COMPLETE ARCHITECTURE WITH CORE FIXES
# ==============================================================================

# --- DEPENDENCY REGISTRATION ---
import streamlit as st  # The core web framework used to render UI elements natively from Python scripts.
import random           # Python's native randomizer module used to shuffle players and randomize drawn pairings.
import uuid             # Universal Unique Identifier module used to generate unforgeable, distinct string keys for matches.
import pandas as pd     # Data analysis library used to convert Python dictionaries into highly structured frontend tables.
import itertools        # Iterator toolkit used to dynamically compute combinations of unique team matchups without duplicates.
import datetime         # Used to record standard timestamps and creation dates for administration telemetry profiles.
import shelve           # Local key-value database system built into Python that saves data persistently to your machine's drive.
...
# DB_FILE dictates the root string name of the persistent file generated on your hard drive by the 'shelve' module.
...
def get_local_data(room_name):
    """
    Retrieves the complete data dictionary payload for a specific room name from the local database file.
    If the room string key does not exist inside the shelve file, it initializes a brand new database schema layout.
    """
    with shelve.open(DB_FILE) as db:
        if room_name not in db:
            # Construct a pristine, empty baseline state dictionary layout tracking all fundamental system parameters.
            default_data = {
                "players": [],              # A simple Python array list tracking all text names of registered club players.
                "teams": [],                # A nested multi-dimensional array list containing paired combinations of player names.
                "matches": [],              # An array list holding individual match metadata dictionaries (IDs, scores, status codes).
                "expenses": {},             # A mapping dictionary structure tracking text player names directly to float monetary balances.
                "ind_leaderboard": {},      # A tracking dictionary computing running statistics for individual profiles.
                "team_leaderboard": {},     # A tracking dictionary calculating cumulative Win and Point totals for distinct paired tandems.
...
def save_local_data(room_name, data):
    """
    Overwrites the persistent disk storage block for a specified room name with a new updated state payload.
    """
    with shelve.open(DB_FILE) as db:        
        db[room_name] = data                
...
if st.session_state.room_id == "ADMIN_PANEL":
    ...
    with shelve.open(DB_FILE) as db:                        
        for r_id in db.keys():                              
            ...
            admin_summary_data.append({
                ...
                "Total Dashboard Openings": traffic_hits,
    ...
    if admin_summary_data:                                  
        ...
        st.download_button(                                 
            label="📥 Download Database Metadata CSV Report",
            data=csv,
            file_name="badminton_clubhouse_local_audit.csv",
            mime="text/csv"
        )
    else:
...
with col2:
    if st.button("Change Room / Exit", use_container_width=True): 
        ...
        if "room_data" in st.session_state:
            del st.session_state.room_data                  
        st.rerun()
...
def generate_and_lock_teams(match_format):
    """
    Core Matchmaking Logic: Ingests the current active player list array, shuffles the index positions randomly, 
    and groups them into structured Singles arrays or Doubles arrays within session state records.
    """
    shuffled_pool = list(st.session_state.room_data["players"]) 
...
def log_match_to_history(match):
    """
    Historical Ledger Logic: Fired precisely when a court game cross-evaluates as completed.
    Parses scores, counts players to track format (Singles/Doubles), and increments career statistics.
    """
    ...
    # 📋 Structural Check: Baseline verification fields for the Team Leaderboard maps.
    if t_a_name not in team_lb: team_lb[t_a_name] = {"Wins": 0, "Points": 0}
    if t_b_name not in team_lb: team_lb[t_b_name] = {"Wins": 0, "Points": 0}
        
    # 📋 Dynamic Expansion: Setup default structured profile dictionary layout fields for standalone player metrics.
    for p in match["team_a"] + match["team_b"]:
        ...
        # Injects backward compatibility support slots for older database files if missing
    ...
    # Compile base scoring arrays directly into data targets
    ...
    # 🏸 Format Evaluation Step: Compute match categorization by tracking court size dimensions
    ...
    # Tick match play count trackers for everyone who stepped onto the playing court floor
    for p in match["team_a"] + match["team_b"]:
        if is_singles_match:
            ind_lb[p]["Singles Played"] += 1
        elif is_doubles_match:
            ind_lb[p]["Doubles Played"] += 1

    # Win-State Determination and Championship Allocation
    if match["score_a"] > match["score_b"]:                     
        team_lb[t_a_name]["Wins"] += 1                          
        for p in match["team_a"]: 
            ind_lb[p]["Wins"] += 1        
            if is_gf:
                ind_lb[p]["Grand Finals Won"] += 1
    else:                                                       
...
# ==============================================================================
# 👥 TAB WORKSPACE MODULE 1: CLUB ROSTER & FINANCIAL EXPENSES
# ==============================================================================
if selected_tab == "👥 Roster & Expenses":
    ...
    with col_p:
        ...
        for idx, player in enumerate(st.session_state.room_data["players"]):
            ...
            if col_del.button("🗑️", key=f"del_{player}_{idx}"):  
                ...
                if player in st.session_state.room_data["expenses"]:
                    del st.session_state.room_data["expenses"][player] 
                save_local_data(room_code, st.session_state.room_data) 
                st.rerun()
    ...
    with col_e:
        ...
        if st.session_state.room_data["expenses"] and st.button("🗑️ Reset Expense Ledger", type="secondary", use_container_width=True):
            for player in st.session_state.room_data["expenses"]:
                st.session_state.room_data["expenses"][player] = 0.0 
            save_local_data(room_code, st.session_state.room_data) 
            st.toast("💰 Expense records cleared back to $0.0!") 
            st.rerun()

# ==============================================================================
# 🎮 TAB WORKSPACE MODULE 2: MATCHMAKING DRAW GENERATION & COURT CONSOLE
# ==============================================================================
    ...
    with col_cfg:
        ...
        match_type = st.radio("Format:", ["Singles", "Doubles"]) 
        ...
        with col_btn1:
            if st.button("👥 Lock Teams", use_container_width=True, type="secondary"):
                ...
                if len(st.session_state.room_data["players"]) < req: 
                    st.error(f"Need at least {req} players!")    
                else:
        ...
        if num_teams >= 2:
            all_pairings = list(itertools.combinations(range(num_teams), 2)) 
            simulated_counts = {i: 0 for i in range(num_teams)} 
            
            for i in range(int(m_count)):
                pair = all_pairings[i % len(all_pairings)]       
            ...
            if len(unique_match_frequencies) <= 1:              
                st.success("💪 Perfect Balance! All teams play an identical number of matches.")
            else:                                               
                ...
                st.warning(f"⚠️ **Uneven Play Warning:** Some teams play fewer matches:\n" + "\n".join([f"- {t}" for t in shortchanged_teams]))
        ...
        if st.button("🎲 Draw Random Matches", use_container_width=True):
            if len(st.session_state.room_data["teams"]) < 2:
                ...
                all_pairs = list(itertools.combinations(st.session_state.room_data["teams"], 2)) 
        ...
        if st.button("🏆 Start Tournament Pack", use_container_width=True, type="primary"):
            if len(st.session_state.room_data["teams"]) < 2:
                ...
                fixtures.append({
                    ...
                    "score_a": 0, "score_b": 0, "max_points": int(final_pts) 
                })
                st.session_state.room_data["matches"] = fixtures
                save_local_data(room_code, st.session_state.room_data)
                st.rerun()
    ...
    with col_play:
        ...
        for match in st.session_state.room_data["matches"]:
            if not match.get("is_final", False) and (match["score_a"] >= match["max_points"] or match["score_b"] >= match["max_points"]):
                ...
                if match["score_a"] > match["score_b"]:
                    current_pack_stats[t_a_str]["Wins"] += 1     
                else:
                    current_pack_stats[t_b_str]["Wins"] += 1     

        sorted_pack_teams = sorted(
            current_pack_stats.values(), 
            key=lambda x: (x["Wins"], x["Points"]), 
            reverse=True                                         
        )

        # --- SCOREBOARD INTERFACE RENDER LOOP ENGINE ---
        for idx, match in enumerate(st.session_state.room_data["matches"]):
            ...
            # --- EVALUATE MATCH TERMINATION THRESHOLDS ---
            if match["score_a"] >= match["max_points"] or match["score_b"] >= match["max_points"]:
                ...
                if match.get("is_final", False):                 
                    ...
                    st.success("✅ Match Completed & Saved to Leaderboard!")
            st.divider()                                         

# ==============================================================================
# 🏆 TAB WORKSPACE MODULE 3: SYSTEM ARCHIVE CAREER LEADERBOARDS
# ==============================================================================
    ...
    with col_l1:
        ...
        # 🛠️ ADVANCED REALTIME AGGREGATION PATTERN FOR EXTENDED STATISTICS
        display_profiles = {}
        
        # Pull down master records ensuring every registered club member is accounted for
        ...
        if display_profiles:                                            
            df_ind = pd.DataFrame.from_dict(display_profiles, orient='index')
            # Sort strategically: Grand Finals won takes top honor, followed cleanly by total wins
            df_ind = df_ind.sort_values(by=["Grand Finals Won", "Wins", "Total points scored"], ascending=[False, False, False])
            st.dataframe(df_ind, use_container_width=True)       
        else:
            st.info("No recorded individual statistics available.")
    ...
    with col_l2:
        ...
        if team_stats:
            ...
            st.info("No recorded team statistics available.")
    ...
    with col_r1:
        if st.button("⚠️ Hard Reset Leaderboards", use_container_width=True, type="secondary"):
            ...
            save_local_data(room_code, st.session_state.room_data) 
            st.success("Leaderboards wiped clean!")
            st.rerun()
